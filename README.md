...
- Stream Deck CLI (for packaging): `npm install -g @elgato/cli@latest`
...
To create a `.streamDeckPlugin` installer file that can be shared with others:

1. **Install the Stream Deck CLI** (if not already installed):
   ```bash
...
3. **Package the plugin**:
   ```bash
   streamdeck pack com.simon-poirier.strava-stats.sdPlugin
   ```

This creates `com.simon-poirier.strava-stats.streamDeckPlugin` that users can double-click to install.

### Ignoring Files During Packaging

Create a `.sdignore` file in the root directory to exclude files from the package (uses `.gitignore` syntax):
...
For Marketplace distribution, you can enable DRM protection to:
- Encrypt files and prevent unauthorized execution
- Verify file integrity to protect users from tampering
- Unlock new SDK features

**To enable DRM:**
1. Update `manifest.json`: Set `SDKVersion` to `3`
2. Update `manifest.json`: Set `Software.MinimumVersion` to `"6.9"` or higher
3. Update SDK: `npm install @elgato/streamdeck@latest`

**Note**: DRM protection only takes effect after uploading to [Maker Console](https://maker.elgato.com/). Learn more at [Stream Deck Distribution Docs](https://docs.elgato.com/streamdeck/sdk/introduction/distribution/).

### Publishing to Marketplace

To publish on the [Stream Deck Marketplace](https://marketplace.elgato.com/stream-deck/plugins):

1. **Review Guidelines**: Ensure your plugin meets [Stream Deck plugin guidelines](https://docs.elgato.com/guidelines/streamdeck/plugins/metadata)
2. **Create Assets**:
   - App icon (for the Marketplace listing)
   - Gallery items (screenshots showcasing functionality)
3. **Submit via Maker Console**:
   - Create an account at [maker.elgato.com](https://maker.elgato.com/)
   - Upload your `.streamDeckPlugin` file
   - Add product details, screenshots, and description
   - Submit for review

Learn more: [Become a Maker Guide](https://docs.elgato.com/makers/general/become-a-maker)

## OAuth 2.0 Implementation (Advanced)

For production use, you should implement proper OAuth 2.0:
...
### "Config Needed" Message
- You haven't entered a Strava access token yet
- Enter your token in the property inspector

### "Error Check Token" Message
- Your access token may have expired
- Your access token may not have the required scopes
- Check your internet connection
- Verify your token at [Strava API Settings](https://www.strava.com/settings/api)

### "No Activity" Message
- You haven't recorded any activities yet
- The filter type might be excluding your activities

### Actions Not Updating
- Check your internet connection
- Verify your Strava account has the activities visible
- Check the plugin logs in the Stream Deck console

## API Reference

This plugin uses:
- **Stream Deck SDK v2.0** - [Documentation](https://docs.elgato.com/streamdeck/sdk/)
...
- Your Strava access token is stored locally on your computer only
- No data is sent to third-party servers (except Strava API)
- The plugin only reads data, it doesn't modify your Strava activities
...
- **Stream Deck SDK**: [Elgato Marketplace Makers Discord](https://discord.gg/GehBUcu627)
- **Strava API**: [Strava Developers Community](https://communityhub.strava.com/)
